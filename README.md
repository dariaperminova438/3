# Анализ разных алгоритмов сортировки и поиска на разных языках программирования (Перминова Дарья)
# Сортировка выбором 
...
1. Алгоритм попарно сравнивает все элементы массива.
2. Сортировка происходит прямо в исходном массиве без создания копий данных.
...
1. Алгоритм многократно сравнивает и попарно обменивает соседние элементы.
2. Все операции обмена выполняются внутри исходного массива без дополнительной памяти.
...
1. Каждый новый элемент последовательно сравнивается с уже отсортированной частью массива.
2. Все сдвиги элементов выполняются внутри исходного массива.
...
1. Алгоритм рекурсивно делит массив пополам и затем сливает отсортированные части.
2. Требуется дополнительный массив для слияния отсортированных половин.
...
1. Алгоритм сравнивает элементы на разных расстояниях, постепенно уменьшая шаг.
2. Все перестановки выполняются внутри исходного массива.
...
1. Массив делится на части относительно опорного элемента, операция повторяется рекурсивно.
2. Глубина рекурсии требует дополнительной памяти в стеке вызовов.
...
1. Алгоритм строит бинарную кучу и последовательно извлекает элементы.
2. Все операции выполняются внутри исходного массива.
...
1. Алгоритм последовательно перебирает все элементы массива до нахождения нужного.
2. Поиск ведется непосредственно в исходной структуре данных.
...
1. На каждом шаге алгоритм делит отсортированный массив пополам.
2. Для работы используются только переменные-указатели без копирования данных.
...
1. Алгоритм предсказывает позицию элемента по формуле интерполяции.
2. Поиск ведется в исходном массиве без дополнительных структур.
...
1. Алгоритм использует числа Фибоначчи для деления массива на части, что гарантирует исключение примерно 1/3 элементов на каждом шаге и логарифмическую сложность.
2. Для работы требуется только хранение нескольких чисел Фибоначчи и индексов, поэтому дополнительная память не нужна.
