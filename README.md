...
Алгоритм всегда просматривает все неотсортированные элементы для поиска минимума, что требует n*(n-1)/2 сравнений — отсюда O(n²). При этом он лишь переставляет элементы в исходном массиве, используя фиксированное число переменных — отсюда O(1).
...
Сортировка обменом имеет сложность O(n²) из-за многократного сравнения соседних элементов, и O(1) памяти, так как обмен происходит внутри массива без дополнительных структур.
...
Сортировка вставками имеет сложность O(n²) в худшем случае из-за последовательного сравнения и сдвига элементов, но O(1) памяти, так как все операции происходят внутри исходного массива без дополнительных затрат памяти.
...
Сортировка слиянием имеет сложность O(n log n) благодаря рекурсивному разделению массива пополам, но требует O(n) дополнительной памяти для хранения временных подмассивов в процессе слияния отсортированных частей.
...
Сортировка Шелла имеет сложность от O(n log²n) до O(n²) в зависимости от выбранных шагов, и O(1) памяти, так как является модификацией сортировки вставками и работает внутри исходного массива без дополнительных структур данных.
...
Время O(n log n): Массив многократно делится пополам (log n раз), каждый раз выполняя обход всех элементов (n).

Время O(n²) в худшем случае: Если опорный элемент всегда выбирается минимальным или максимальным, деление становится неравномерным.

Память O(log n): Рекурсивные вызовы создают log n уровней стека из-за разделения массива пополам.
...
Пирамидальная сортировка имеет сложность O(n log n) при любых данных, так как она сначала строит из массива бинарную кучу, а затем последовательно извлекает элементы, каждый раз перестраивая кучу. Это требует log n операций для каждого из n элементов. Пространственная сложность составляет O(1), поскольку алгоритм переставляет элементы непосредственно в исходном массиве, используя лишь несколько дополнительных переменных, и не требует дополнительной памяти.
...
Последовательный поиск имеет временную сложность O(n), так как в худшем случае требуется проверить все n элементов массива. Пространственная сложность составляет O(1), поскольку алгоритм использует только фиксированное количество переменных и не требует дополнительной памяти beyond исходной структуры данных.
...
Время O(log n): На каждом шаге поиска область поиска уменьшается вдвое. Даже для массива из миллиарда элементов достаточно ~30 итераций.

Память O(1): Алгоритм использует только несколько переменных для хранения границ поиска и среднего элемента, не создавая новых структур данных.
...
Интерполирующий поиск угадывает позицию элемента по значениям, поэтому в среднем очень быстрый (O(log log n)). Но если данные "кривые", может работать медленно как линейный поиск (O(n)). Память O(1) — алгоритм не использует дополнительную память, работая только с исходным массивом и несколькими переменными.
...
Время O(log n): Алгоритм делит массив на части, пропорциональные числам Фибоначчи, что гарантирует не более log n шагов.

Память O(1): Используются только несколько переменных для хранения чисел Фибоначчи и индексов, без создания дополнительных структур данных.
