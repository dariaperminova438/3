...
Алгоритм поиска Фибоначчи имеет временную сложность O(log n), что аналогично бинарному поиску. Это достигается за счет того, что на каждом шаге алгоритм уменьшает область поиска, используя числа Фибоначчи для определения точек разделения. Количество сравнений пропорционально логарифму от размера массива. Пространственная сложность составляет O(1), так как алгоритм использует только фиксированное количество переменных для хранения чисел Фибоначчи и индексов, не зависящее от размера входных данных.
